...
Al crear PRs con `gh pr create`, **hazlo justo después del push**, sin esperar confirmación adicional del usuario (el push ya fue confirmado).

Formato obligatorio:

```
gh pr create --title "feat(#<id>): <descripción breve>" --body "$(cat <<'EOF'
## Cambios

- `ruta/archivo` (nuevo|modificado): qué hace o qué cambió.
- ...

## Tests

- `tests/test_<módulo>.py` (nuevo|modificado): qué cubren los tests añadidos.

## Verificación

Suite completa pasa: `./venv/bin/pytest -q` → exit code 0.

## Siguiente feature desbloqueada

#<id> (`<nombre>`) — o "Ninguna nueva" si no aplica.
EOF
)"
```

Reglas:
- ❌ **No incluyas** ninguna firma de autoría ("Generated by Claude" ni similar).
- El título sigue el mismo formato que el commit: `feat(#<id>): <descripción breve>`.
- El body en español, conciso, solo información útil para el revisor.
...
4. Solo si el usuario confirma — **en este orden exacto**:
   ...
   d. Crea la PR con `gh pr create` siguiendo el formato de la sección "Pull Request" de arriba.
